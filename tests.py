"""
The design of the code for the permutation test is partially based on: https://github.com/ikjunchoi/rff-mmd from https://arxiv.org/pdf/2407.08976.
"""
...
def MMDbtest(
    ...
    """
    Perform a biased MMD permutation test with full RBF kernel.

    Supports different sample sizes n_x != n_y.

    Parameters
    ----------
    Z : ndarray of shape (n_x + n_y, d)
        Data matrix with both samples stacked.
        First n_x rows are from group X, next n_y rows from group Y.
    n_x : int
        Number of samples in group X.
    n_y : int
        Number of samples in group Y.
    bw : float
        Bandwidth (sigma / lengthscale) parameter for the RBF kernel.
    seed : int, optional
    ...
    plot : bool, default=False
        If True, plot permutation null and test threshold.

    Returns
    -------
    output : int
        1 if the null hypothesis "P == Q" is rejected at level alpha,
        0 otherwise.
    dt : float
    ...
    if n_tot != (n_x + n_y):
        raise ValueError(
            f"Inconsistent sizes: Z has {n_tot} rows but n_x + n_y = {n_x + n_y}"
        )

    # 1. Precompute kernel once
    K = RBFkernel(Z, Z, bw)  # shape (n_tot, n_tot)

    # 2. Observed statistic using the provided split [0:n_x] vs [n_x:...]
    t_obs = MMD2b_from_K(K, n_x)

    # 3. Permutation null distribution
    H0 = np.empty(B + 1, dtype=float)
    for i in tqdm(range(B)):
        perm = rng.permutation(n_tot)
        A = perm[:n_x]              # choose n_x points as "group X"
        H0[i] = MMD2b_from_K(K, n_x)

    # last entry is observed (unpermuted)
    H0[-1] = t_obs
    ...
    if plot:
        ...
        plt.vlines(thr, 0, max(values) * 0.8, color='red',
                   label=f"{alpha}-level threshold")
        plt.vlines(t_obs, 0, max(values) * 0.8, color='black',
                   label="Observed test statistic")
        plt.legend()
        plt.show()

    dt = time.time() - start
    return output, dt, n_tot


def rMMDtest(Z, nx, ny, seed=None, bandwidth=1, alpha=0.05, kernel='gaussian', R=20, B=199):
    """
    Performs the Random Fourier Feature approximation MMD test.
    
    Parameters:
        Z (array): Feature matrix of shape (n, d).
        nx (int): size of first sample Z[:nx]
        ny (int): size of second sample Z[nx:ny]
    ...
    if ntot != (nx + ny):
        raise ValueError(
            f"Inconsistent sizes: Z has {ntot} rows but n_x + n_y = {nx + ny}"
        )
    ...
    I_1 = np.concatenate((np.ones(nx), np.zeros(ny)))
    I = np.tile(I_1, (B + 1, 1))
    ...
    bar_Z_B_piX = (1 / nx) * I_X @ psi_Z
    bar_Z_B_piY = (1 / ny) * I_Y @ psi_Z
...
def NysMMDtest(Z, nx, ny, seed=None, bandwidth=1, alpha=0.05, method='uniform', k=20, B=199):
    """
